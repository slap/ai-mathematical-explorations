...
📄 **[Read the note (PDF, 10 pages)](exploration.pdf)** · [LaTeX source](source.tex) ·
...
divided by the Vandermonde determinant `V` of those roots. That the ratio is a polynomial is
not news — it is the subresultant itself, up to a power of a leading coefficient — but the
formula does not exhibit it as one. **The AI-generated note published here** gives explicit
quotient-free expressions for it, and reasonably pleasant ones: `P_S = |O_S| / V` is a
multi-Schur function of Lascoux in two alphabets — the roots of `g` and the difference of the
two root alphabets — in three equivalent forms. In the classical case it collapses to a single
supersymmetric Schur function indexed by a rectangle, which recovers a theorem of Lascoux and
Pragacz; a determinant of divided differences settles multiple roots; and the naive
multivariate analogue is shown to be false.

### Research context

The question I put to the model was whether the rational expressions of [DKS05] can be
replaced by formulas without denominators, and what those polynomials look like. I gave it
that problem statement, split into four lines — the divisibility, an explicit formula for the
quotient, its expression in a natural basis of symmetric functions, and the extension to
multiple roots and to the multivariate case — and then only asked it to close the third line
before moving on, and to collect what it had into a note. Beyond that initial prompt there was
no intervention of mine: the approach, the proofs, the computations and the write-up are
entirely the model's. The same session produced
...
The content is in the explicit forms; the divisibility itself is the elementary step, since
`P_S` is the subresultant up to a power of `b_{d₂}` and is therefore a polynomial for free.

> **Theorem 3.2 (divisibility over ℤ).** `V` divides `|O_S|` in `ℤ[a₀,…,a_{d₁}][ξ₁,…,ξ_{d₂}]`,
> and the quotient `P_S` is symmetric in the roots.
>
...
- **The note's own §11.** Every statement was tested by symbolic computation by the AI
...
- **What the AI flagged itself.** The bibliographic data of [Hong], [DKS13], [DHKS] and
  ...
  the source. The note does not claim novelty for Theorem 6.1, which it presents as
...
`H₀ = a_{d₁} ≠ 1`, which makes the *size* of every Jacobi–Trudi determinant in the note
matter.
...
- [exploration.pdf](exploration.pdf) — the note, 10 pages
...
prior work where these formulas, or stronger ones, already appear (the note itself asks this
