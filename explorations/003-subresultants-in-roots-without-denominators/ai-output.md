...
**Session.** Claude Opus 5 (`claude-opus-5`), Anthropic, through Claude Code, 15 September
2026, in the working directory of my project on subresultant formulas without denominators.

**What is not included.** The intermediate turns of the same session — some three hours of
...
session, which moved on to the Casas–Alvero conjecture and produced
[exploration 001](../001-algebraic-proof-of-ghosh-lemma-3-7/) and
[exploration 002](../002-non-redundancy-of-the-casas-alvero-conditions/).

The LaTeX source at the end is the file as the AI wrote it, including the three small
presentational changes it made to it two days later (see the note before it). The
`source.tex` published alongside this file differs from it only by the editorial changes
listed in its header.

---

**Prompt** — 2026-09-15 16:52

> quiero empezar un nuevo proyecto para formulas de resultantes sin denominadores, concretamente este problema: Problema
>
> Las fórmulas de subresultantes en términos de las raíces desarrolladas por D'Andrea, Krick y Szanto aparecen como cocientes de determinantes, con denominadores dados por determinantes de Vandermonde.
>
> ¿Es posible reemplazar estas expresiones racionales por fórmulas universalmente polinómicas en las raíces, sin denominadores, y obtener una descripción explícita de dichos polinomios?
>
> En particular:
>
> demostrar la divisibilidad del numerador por el Vandermonde;
> obtener una fórmula explícita para el cociente;
> expresar el cociente en una base natural de polinomios simétricos;
> estudiar la extensión de estas fórmulas al caso de raíces múltiples y al caso multivariado. podes darme un primer approach de lineas a seguir y definimos por donde encararlo? arma en esta carpeta lo que sea necesario para el proyecto

*(Translation: "I want to start a new project on resultant formulas without denominators,
specifically this problem: **Problem.** The subresultant formulas in terms of the roots
developed by D'Andrea, Krick and Szanto appear as quotients of determinants, with denominators
given by Vandermonde determinants. Is it possible to replace these rational expressions by
universally polynomial formulas in the roots, without denominators, and to obtain an explicit
description of those polynomials? In particular: prove the divisibility of the numerator by
the Vandermonde; obtain an explicit formula for the quotient; express the quotient in a
natural basis of symmetric polynomials; study the extension of these formulas to the case of
multiple roots and to the multivariate case. — can you give me a first approach, lines to
follow, and we decide where to start from? set up in this folder whatever the project needs".)*

---

**Prompt** — 2026-09-15 18:53

> cerremos la linea 3, antes de pasar a multivariado

*(Translation: "let's close line 3, before moving on to the multivariate case".)*

---

**Prompt** — 2026-09-15 19:28

> podes juntar todo lo que obtuviste en un paper en latex?

*(Translation: "can you put everything you got into a paper in LaTeX?".)*

---

**Claude** — 2026-09-15 19:28–19:36

Voy a armar el paper. Primero veo con qué compilo.

MiKTeX está. Antes de escribir, quiero verificar una idea que convertiría el caso de raíces múltiples en un teorema demostrado: que `P_S` es el determinante de **diferencias divididas**.

El lema de Newton funciona — eso convierte el caso de raíces múltiples en teorema demostrado. Ahora sí, el paper.

Compila: 10 páginas. Segunda pasada para resolver referencias cruzadas y reviso avisos.

Limpio: sin referencias indefinidas ni cajas desbordadas. Reviso el resultado visualmente.

El Ejemplo 8.3 se sale del margen (la tabla de 4 columnas es muy ancha). Lo reformateo.

Una cosa que quiero dejar rigurosa: la prueba del corolario confluente estaba muy esquemática. La reescribo con el argumento de grupos de columnas, que sí cierra.

Listo: `paper/subresultants-no-denominators.tex`, 10 páginas, compila limpio (dos pasadas de `pdflatex`, cero cajas desbordadas, cero referencias sin resolver).

## Contenido

**7 teoremas demostrados**, no sólo verificados:

1. **Divisibilidad** — vía el lema "se anula en las diagonales ⟹ divisible por el Vandermonde" sobre un dominio íntegro, así que vale sobre `ℤ`.
2. **Expansión de Schur** — Cauchy–Binet sobre `O_S = ρ·W`.
3. **Forma multi-Schur con el signo exacto** — la prueba clave: `O_S = ρ·W` y `M = ρ·Y` comparten la matriz de coeficientes `ρ`, y todo se reduce a comparar el bialternante contra el Jacobi–Trudi de la misma `λ(E)`.
4. **Rectángulo** — el caso clásico colapsa a `s_{((d₁-k)^(d₂-k))}(Ξ−A)`; recupera Lascoux–Pragacz.
5. **Toeplitz/Padé** — el coeficiente principal como determinante de Toeplitz de la serie de `f/g` en el infinito.
6. **Base natural para `S` arbitrario** — coeficientes Schur sesgadas `s_{λ/μ}(A)`, con criterio de anulación (Corolario 8.2).
7. **Forma de Newton** — `P_S = det(R_i[ξ₁,…,ξ_j])`.

Más la **Proposición 10.1** (el análogo multivariado ingenuo es falso) con el diagnóstico estructural vía armónicos diagonales.

## Dos cosas que agregué al escribir

**El lema de Newton** (Teorema 9.1) no lo tenía: `P_S` es el determinante de las **diferencias divididas** de las filas. La matriz de cambio de base de evaluaciones a diferencias divididas es triangular con determinante `1/V`, así que sale en dos líneas — y como las diferencias divididas en nodos repetidos son derivadas, el caso de raíces múltiples pasa de "verificado" a **demostrado**. Lo verifiqué simbólicamente antes de escribirlo.

Y reescribí la prueba del corolario confluente, que me había quedado a mano alzada: el argumento correcto es partir las raíces en grupos, hacer las operaciones de columna dentro de cada grupo (lo que divide por `∏ V^{(i)}`), partir `V` en factores intra- e inter-grupo, y tomar el límite. Ahí sí cierra.

## Honestidad sobre el alcance

Hay una sección explícita (§11, *Status of the statements*) que separa lo demostrado de lo verificado, y una tabla con el rango exacto de cada verificación. La validación punta a punta contra la definición clásica (§11 final) está destacada como el único chequeo que no toma la fórmula de DKS como hipótesis.

## Antes de circularlo

Dos cosas que no podía resolver yo, marcadas con `% TODO` en el `.tex` y en el README: **los autores** (dejé `\author{}` vacío en vez de inventar), y **los datos bibliográficos exactos** de [Hong], [DKS13], [DHKS] y [DKS15] — los puse de memoria y algunos números de volumen/página pueden estar mal. El de Hong en particular lo cité como technical report sin año porque no lo tengo confirmado.

Lo escribí en inglés, que es lo estándar en esta literatura; si lo querés en castellano lo paso.

---

**Note on the 17 September changes.** On 2026-09-17, in the same session, the model made
three presentational changes to the file while going over the repository: it moved two
in-line matrices into display mode (§8 and §9) and translated its own `% TODO` comment
before the bibliography into English. Nothing mathematical changed; `git diff` of those two
commits in the research repository is eleven insertions and seven deletions, all of them in
those three places. The source below is the file after them.

---

## The LaTeX source, as written
...
determinant in the roots. We show that the numerator is always divisible by the
Vandermonde determinant over $\ZZ$, and we describe the quotient explicitly.
...
coefficients of $f$. For subresultants the known analogues are not polynomial:
...
This paper answers the following questions. Is the numerator $|\cO_\cS|$ always
divisible by $\cV_{d_2}$, so that \eqref{eq:dks} can be replaced by a formula
which is universally polynomial in the roots? If so, what is the quotient, and in
which basis of symmetric functions is it naturally expressed? Do the answers
extend to multiple roots and to the multivariate setting?
...
\]
\begin{itemize}
\item \textbf{Divisibility} (Theorem~\ref{thm:div}). Each column of $\cO_\cS$
